...
const LeaveTable = () => {
  ...
  useEffect(() => {
    ...
    const fetchLeaves = async () => {
      try {
        ...
        setLeaves(data); // Directly set the fetched data, assuming it has the 'status' field
  ...
  const handleApprove = async (id) => {
    try {
      ...
      // Update leaves left in the employee record
      await axios.put(`http://localhost:3001/api/empManagement/${employee._id}`, {
        leavesLeft: updatedLeavesLeft,
      });

      // Update the leave's status to "Approved" in the backend
      await axios.put(`http://localhost:3001/api/employeeProfile/${id}`, {
        status: "Approved",
      });

      // Update the leave's status in the frontend state
  ...
  const handleReject = async (id) => {
    try {
      // Update the leave's status to "Rejected" in the backend instead of deleting
      await axios.put(`http://localhost:3001/api/employeeProfile/${id}`, {
        status: "Rejected",
      });
    ...
    <div className="bg-white-100">
      <div className="container mx-auto p-4">
        <div className="bg-white shadow-lg rounded-lg">
          <h1 className="text-2xl font-bold text-gray-800 mb-4 text-left">Leave Requests</h1>
            ...
            <table className="min-w-full bg-white border border-gray-200">
              <thead>
                ...
                >
                  ...
                  <th className="py-3 px-4 text-left w-1/12">Status</th> {/* Status Column */}
                          ...
                          ? employees.map(
                              (emp) =>
                                emp.email === leave.Email && (
                                  <td className="py-3 px-4" key={emp.email}>
                                    {emp.leavesLeft}
                                  </td>
                                )
                            )
      ...
      {/* Approve Dialog */}
      <Dialog open={openApproveDialog} onClose={() => handleApproveDialogClose(false)}>
      ...
      {/* Reject Dialog */}
      <Dialog open={openRejectDialog} onClose={() => handleRejectDialogClose(false)}>
